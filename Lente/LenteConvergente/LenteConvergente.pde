...
void draw() {
    ...
    if(mouseY < this.height/2){//Imagem na parte de cima
      ...
      if(pObjeto.x<pFoco.x){//imagem antes do foco
        if(alturaMaxima>vObjeto){
          ...
          line(this.width/2, pObjeto.y, this.width, equacaoRetaY(this.width/2, pObjeto.y, pFoco2.x, pFoco2.y, this.width));
        }else{
          tamanhoCerto = false;
          text("Objeto Maior que o espelho", this.width/1.30, 65);
          stroke(0, 255, 0);//cor das linhas de reflexão
          line(pObjeto.x, pObjeto.y, this.width, equacaoRetaY(pObjeto.x, pObjeto.y, this.width/2, pObjeto.y, this.width));//topo do objeto ate espelho
        }
        
        if(alturaMaxima>-vImagem){
          stroke(0, 255, 0);//cor das linhas de reflexão
          line(pObjeto.x, pObjeto.y, this.width/2, this.height/2);
          stroke(0, 255, 255);//cor das linhas de refletidas
          line(this.width/2, this.height/2, this.width, equacaoRetaY(pObjeto.x, pObjeto.y, this.width/2, this.height/2, this.width));//ponto 2 do espelho ate o canto
        }else{
          tamanhoCerto = false;
        ...
        if(tamanhoCerto){
          ...
          if(Math.abs(vObjeto) < Math.abs(vImagem)){
            text("Imagem: Real, Invertida e Maior", this.width/1.30, 65);
          }else if(Math.abs(vObjeto) > Math.abs(vImagem)){
        ...
        if(alturaMaxima>vObjeto){
          ...
          stroke(0, 255, 0);//cor das linhas de reflexão
          line(pObjeto.x, pObjeto.y, this.width/2, pObjeto.y);//topo do objeto ate a lente
          ...
          line(this.width/2, pObjeto.y, this.width, equacaoRetaY(this.width/2, pObjeto.y, pFoco2.x, pFoco2.y, this.width));//espelho ate foco do lado direito
          ...
          stroke(0, 255, 255);//cor das linhas refletidas
          line(this.width/2, pObjeto.y, pImagem.x, pImagem.y);//Ponto 1 do espelho até o topo da imagem
          line(pObjeto.x, pObjeto.y, pImagem.x, pImagem.y);//topo do Objeto ate o topo da imagem
        }else{
          tamanhoCerto = false;
          stroke(0, 255, 0);//cor das linhas de reflexão
          text("Objeto Maior que o espelho", this.width/1.30, 65);
  ...
  arc(this.width/2 - raio + 4, this.height/2, raio*2, raio*2, -HALF_PI/9, HALF_PI/9);//arco(Espelho) formando no máximo 10 Graus com o centro
